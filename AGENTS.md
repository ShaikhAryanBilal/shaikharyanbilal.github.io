...
Content lives in `content/*.json` (regenerated by `scripts/migrate.mjs` from the WordPress XML export in `plans/`). All page copy is driven by these files — edit JSON, not components.
